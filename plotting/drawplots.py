...
def compute_ResolutionvsX(h2d):
    ...
    for ctr, h in enumerate(h2d):
        ...
        for i in range(h.GetNbinsX()+1):
            ...
            proj.Fit(f_gaus,"Q")
            #proj.Draw()
            #c1.SaveAs('/afs/cern.ch/work/p/pdas/www/L1DPG/JECval/bin_'+str(i)+'.png')
            #c1.SaveAs('/afs/cern.ch/work/p/pdas/www/L1DPG/JECval/bin_'+str(i)+'.pdf')
            if f_gaus.GetParameter(1) >0 and f_gaus.GetParError(2)/f_gaus.GetParameter(1)<10.03:
                ...
                h_responsevsX.SetBinError(i,proj.GetMeanError())
                h_resolvsX.SetBinContent(i,proj.GetRMS()/proj.GetMean())
                h_resolvsX.SetBinError(i,proj.GetRMSError()/proj.GetMean())
            else:
